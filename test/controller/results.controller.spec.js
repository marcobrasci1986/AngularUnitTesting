describe("ResultsControllerTest", function () {
    ...
    beforeEach(module(function ($exceptionHandlerProvider) {
        //$exceptionHandlerProvider.mode('rethrow'); //default behaviour
    ...
    it("should set result status to error", function () {
        spyOn(MovieService, 'search').and.callFake(function () {
            var deferred = $q.defer();
            deferred.reject('Something went wrong!'); // REJECT promise: mocked data
            return deferred.promise;
        });

        $location.search('q', 'star wars');

        $controller('ResultsController'); // create controller
        $rootScope.$apply();
